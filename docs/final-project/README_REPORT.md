...
**Important:** Final-project screenshots are stored locally in `docs/final-project/screenshots/` but are **not tracked by git** (in `.gitignore`). This keeps the repository size manageable while allowing the LaTeX document to reference local images during compilation.
...
2. Upload all files from `docs/final-project/` directory
