...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{screenshots/lidar/lidar_troubleshooting_false_wall_rviz.png}
    \caption{False wall detection in RViz: LIDAR detecting ground plane when robot pitched forward}
...
\subsection{Issue 1: Frame ID Mismatch (base\_footprint vs base\_link)}
...
\subsection{Issue 4: Transform Authority Conflicts}
...
\begin{figure}[htbp]
    ...
    \caption{Planned vs actual trajectory comparison in PlotJuggler: Red points show planned path, green points show actual executed path with localization drift visible}
...
\textbf{Successful solution:} Create specialized analysis URDF (\texttt{rudimentary\_bot\_analysis.urdf.xacro}) with wheels converted from \texttt{continuous} to \texttt{fixed} joints.
...
Kernel & Linux 6.17.9 \\
CPU & [Unspecified - modern x86-64] \\
GPU & NVIDIA RTX 3080 \\
RAM & [Unspecified - adequate for simulation] \\
\bottomrule
\end{tabular}
\end{table}
